...
export default function ImprovePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { setError('写真のアップロードに失敗しました。'); setPhotoPreview('') }
            ...
            {steps.map((step, i) => (
              <div key={i} className="flex gap-2 items-start">
                ...
                <textarea value={step} onChange={e => setSteps(prev => prev.map((s, j) => j === i ? e.target.value : s))}
                  rows={2} className="flex-1 border border-stone-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" />
